...
pub(crate) trait ViewVecObserverApi<Msg> {
    ...
    // CORE
    fn build(&self, f: &Fn(View<Msg>)->Vec<Dom<Msg>>);
...
pub struct TickArgs<'a, Msg> {
    pub(crate) update: &'a mut FnMut(&mut ViewItem<Msg>),
    pub(crate) removed: &'a mut FnMut(Dom<Msg>),
}
...
enum ViewMapSignal<K, V, Msg> {
    Weak {
        init: ToView<K, V, Msg>,
        state: Weak<RefCell<ObserverState<Msg, K>>>,
    },
    Own {
        state: Rc<RefCell<ObserverState<Msg, K>>>,
    },
}

// impl<T, Msg> MapOpObserver<T> for ViewVecSignal<T, Msg> {
...
pub(crate) struct ForEachSegment<'a, K, Msg> {
    if_view: &'a mut FnMut(&mut HashMap<K, View<Msg>>),
    if_dom: &'a mut FnMut(&mut DomSegment<K, Msg>),
}
...
enum ObserverState<K, Msg> {
    Borrowed,
    Terminated,
    Value(SegmentValue<K, Msg>),
}

enum SegmentValue<K, Msg> {
    View(ViewSegment<K, Msg>),
    Dom(DomSegment<K, Msg>),
}

pub(crate) struct ViewSegment<K, Msg>(pub HashMap<K, View<Msg>>);

pub(crate) struct DomSegment<K, Msg> {
    pub active: HashMap<K, ViewItem<Msg>>,
...
pub(crate) struct ToView<K, V, Msg>(Box<Fn(&K, &V)->View<Msg>>);

impl<K, V, Msg> std::fmt::Debug for ToView<K, V, Msg> {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "ToView")
    }
}
