...
impl Spec for AppSpec {
    ...
    fn init(&self, loaded: InitArgs<Self::Model>, key: &InitKey) -> Init<Self::Model, Self::Msg> {
        ...
        let url_parser: UrlParser<Page> = match_path!(
            ...
            ["account", "account-master"] => {
                Page::Account(AccountPage::AccountMaster)
            }
            _ => {
                Page::NotFound
            }
        ...
        match msg {
            ...
            Msg::UrlRequest(page) => {
                match &page {
                    ...
                    Page::Account(AccountPage::AccountMaster) =>
                        cmd.navigate("/account/account-master"),
                    Page::NotFound =>
    ...
    fn view(&self, model: &Self::Model) -> Html<Self::Msg> {
        let nav_link = move |text: &str, active: bool, on_click: Msg| -> Html<Msg> {html!(li|
            width: "100%"
            text_align: "center"
            padding: "8px"
            user_select: "none"
        ...
        let navigation: Html<Msg> = html!(nav.ul|
            z_index: "2"
            position: "relative"
            margin: "0"
            padding: "0"
        ...
        let homepage = root_page(html!(
            h1(text("Homepage"))
        ));
        let content = root_page(html!(
            h1(text("Content"))
        ));
